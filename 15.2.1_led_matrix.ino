...
const int smilingFace[] = {
 ...
 0x06,0x09,0x79,0x82,0x82,0x79,0x09,0x06
};
...
void loop() {
 // Define a one-byte variable (8 bits) which is used to represent the selected state of
//8 columns.
for (int i = 0; i < 128; i++) { // Repeat for 128 frames for variety
    for (int k = 0; k < 10; k++) { // Repeat each frame 10 times for persistence of vision
      for (int j = 0; j < 8; j++) { // Display each column
        matrixColsVal(0xFF); // Select all columns
        matrixRowsVal(random(255)); // Display random data for each column
        delay(20); // Adjust the delay for desired flame effect
 int cols;
 // Display the static smiling pattern
 for (int j = 0; j < 500; j++ ) { // repeat 500 times
 cols = 0x01; // Assign 0x01(binary 00000001) to the variable, which represents the
//first column is selected.
 ...
 matrixRowsVal(gColsB[i]);
  matrixRowsVal(gColsC[i]);
   matrixRowsVal(gColsD[i]);
    matrixRowsVal(gRowsB[i]);
     matrixRowsVal(gRowsC[i]);
      matrixRowsVal(gRowsD[i]);
 delay(0.2); // display them for a period of time
 ...
 for (int i = 0; i < 128; i++) { // "space，0-9，A-F"16 letters ,each letter hold 8
...
// make latchPin output high level, then 74HC595 will update the data to parallel
//output
